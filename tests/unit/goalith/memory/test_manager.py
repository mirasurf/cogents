...
class TestMemoryManager:
    ...
    def test_search_similar_goals(self, sample_goal_node):
        ...
        different_goal = GoalNode(
            id="different-1", 
            description="Completely different objective",
            type=NodeType.TASK,
            tags={"work", "urgent"}
        )
        
        # Store contexts
        manager.store_context(similar_goal.id, "metadata", {
            "domain": "testing",
            "complexity": "medium"
        })
        manager.store_context(different_goal.id, "metadata", {
            "domain": "production", 
            "complexity": "high"
        })
    ...
    def test_caching_mechanism(self, sample_goal_node):
        ...
        cache_misses_after_first = manager._stats["cache_misses"]
